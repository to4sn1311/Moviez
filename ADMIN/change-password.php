...
if (strlen($_SESSION['username']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = mysqli_query($conn, "SELECT admin_pas FROM admin WHERE admin_acc = '$adminid'");
        $num = mysqli_num_rows($sql);
    
        if ($num > 0) {
            $update_sql = mysqli_query($conn, "UPDATE admin SET admin_pas = '$newpassword' WHERE admin_acc = '$adminid'");
            if ($update_sql) {
